...
按 **START** 录音 → 说一句中文 → 再按 **START** 停止；约 1-2 秒后转写
文字直接进 SSH 终端，效果如同您逐字打出来。整段中文只用嘴说就行，
软键盘可以省略。
...
仓库里两个 installer，CLI 和 3DS 端体验完全一致，区别在**模型推理跑在哪**。

| | **API 轨**（推荐） | **双轨** |
|---|---|---|
| 推理 | OpenRouter Whisper Large V3 Turbo（云端）| OpenRouter（默认）**+** 本地 whisper.cpp `small`（备选）|
| 服务器占用 | ~30 KB | ~600 MB（模型 + venv）|
| 4 秒音频延迟 | ~1-2 秒 | API: ~1-2 秒 · 本地: 2-30 秒（看 CPU）|
| 费用 | $0.04/音频小时（≈ 每分钟 $0.0007）| 走 API 同价 · 走本地免费 |
| 隐私 | 音频上云 | 本地轨音频不出服务器 |
| 是否需联网 | 是（HTTPS 到 openrouter.ai）| API 需 · 本地不需 |
| 服务器 CPU 负载 | 几乎零 | 本地轨载入 ~1 GB 内存 |
...
加上 `python3-venv` 与 ~600 MB 磁盘空间。会下载 `whisper-small`
多语模型并把 daemon 注册到 systemd-user，但**默认仍走 API**：
...
dssh-whisper switch api     # 下次按 START → OpenRouter
